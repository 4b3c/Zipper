#!/bin/bash
# Builds ~/Desktop/Zipper.app — double-click to fetch, serve and open the dashboard.
# Regenerate any time; the bundle is disposable, this script is the source.
set -euo pipefail
VAULT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP="${1:-$HOME/Desktop/Zipper.app}"
...
<plist version="1.0"><dict>
  ...
  <key>CFBundleIdentifier</key><string>com.example.zipper</string>
...
cat > "$APP/Contents/MacOS/Zipper" <<LAUNCH
#!/bin/bash
...
exec "$PY_BIN" Scripts/serve.py --port "\$PORT" --open \\
     >> "\$HOME/Library/Logs/zipper-dashboard.log" 2>&1
LAUNCH

chmod +x "$APP/Contents/MacOS/Zipper"
echo "built $APP"
echo "vault:  $VAULT"
echo "python: $PY_BIN"
